...
void RawCode::FromString(const string& code_str) {
  boost::split(*dynamic_cast<vector<string>*>(this), code_str,
...
bool TableEncoder::DfsEncode(const string& phrase,
  ...
  if (start_pos == phrase.length()) {
    ...
    if (Encode(*code, &encoded)) {
      ...
      if (code->size() == 2 &&
          boost::regex_match(dict_name_, boost::regex("^sbf[mxd]$"))) {
        if (boost::regex_match(
                (*code)[0],
                boost::regex("^[qwrtsdfgzxcvbyphjklnm][aeuio].*|.+1$")) &&
            boost::regex_match(
                (*code)[1],
                boost::regex("^[qwrtsdfgzxcvbyphjklnm][aeuio].*|.+1$"))) {
          return false;
        ...
        if (boost::regex_match((*code)[0], boost::regex("^.+2$")) &&
            boost::regex_match((*code)[1], boost::regex("^.+1$"))) {
          return false;
        }
      }

      if (boost::regex_match(dict_name_, boost::regex("^sbjm|sbfx$"))) {
        collector_->CreateEntry(encoded.substr(3) + " " + phrase,
                                encoded.substr(0, 3), value);
      } else if (boost::regex_match(dict_name_,
                                    boost::regex("^sbf[md]|sbzr|sbxh$"))) {
        collector_->CreateEntry(encoded.substr(4) + " " + phrase,
                                encoded.substr(0, 4), value);
      } else {
