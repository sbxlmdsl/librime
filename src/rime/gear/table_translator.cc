...
an<Candidate> TableTranslation::Peek() {
  ...
  if (phrase) {
    ...
    phrase->set_quality(exp(e->weight) + options_->initial_quality() +
...
TableTranslator::TableTranslator(const Ticket& ticket)
  ...
  if (Config* config = engine_->schema()->config()) {
    ...
    config->GetBool(name_space_ + "/stop_change", &stop_change_);
...
an<Translation> TableTranslator::Query(const string& input,
  ...
  if (dict_ && dict_->loaded() &&
          (!ctx->get_option("is_enhanced") &&
               boost::regex_match(
                   dict_->name(),
                   boost::regex("^fmn|fxn|fmnts|spn||spnts|jmn|jmnts$")) ||
           ctx->get_option("third_pop") &&
               boost::regex_match(dict_->name(),
                                  boost::regex("^sss|jm3|jmsbb$"))) ||
      !ctx->get_option("slow_adjust") &&
          boost::regex_match(dict_->name(), boost::regex("^jm3|jmsbb$")) ||
      ctx->get_option("slow_adjust") &&
          (boost::regex_match(dict_->name(), boost::regex("^jmsbb$")) &&
           code.length() == 1))
    ;
  // else if (boost::regex_match(dict_->name(),
  // boost::regex("^zrzdy|xhzdy|fmzdy|fxzdy|jmzdy$")) && code.length() == 1)
  //	;
  else if (ctx->get_option("fast_pop") && !ctx->get_option("is_enhanced") &&
           engine_->schema()->schema_id() == "sbfx" &&
           boost::regex_match(dict_->name(), boost::regex("^fmn|fmnts|fxn$")))
    ;
  else if (boost::regex_match(dict_->name(), boost::regex("^sbs_b|sps_b$")) &&
           code.length() < 3 && !ctx->get_option("is_hidden"))
    ;
  else if (boost::regex_match(dict_->name(),
                              boost::regex("^s_b|sbs_b|sps_b$")) &&
           engine_->schema()->schema_id() != "sbjm" &&
           code.length() < dict_->name().length() &&
           ctx->get_option("is_hidden"))
    ;
  else if (ctx->get_option("is_enhanced") && ctx->get_option("is_hidden") &&
           boost::regex_match(dict_->name(),
                              boost::regex("^jmnts|fmnts|spnts$")))
    ;
  else if (!ctx->get_option("is_enhanced") &&
           engine_->schema()->schema_id() == "sbjm" &&
           boost::regex_match(dict_->name(), boost::regex("^sbjf$")))
    ;
  else if (boost::regex_match(dict_->name(), boost::regex("^fxn$")) &&
           code.length() < 3)
    ;
  else if (!(ctx->get_option("fixed") || ctx->get_option("mixed") ||
             ctx->get_option("popping")) &&
           engine_->schema()->schema_id() == "sbpy" &&
           boost::regex_match(dict_->name(),
                              boost::regex("^pygd|pygd_lookup$")))
    ;
  else if (!ctx->get_option("is_enhanced") &&
           engine_->schema()->schema_id() == "sbjm" &&
    ...
    DictEntryIterator iter;
    if (dict_ && dict_->loaded() &&
        !boost::regex_match(dict_->name(), boost::regex("^sbjm|sbzr|sbxh$"))) {
      dict_->LookupWords(&iter, code, false);
    }
    UserDictEntryIterator uter;
    if (enable_user_dict) {
      if (!ctx->get_option("is_enhanced") &&
          boost::regex_match(dict_->name(), boost::regex("^sbjm$")) &&
          ((code.length() == 3 &&
            string("1234567890").find(code[2]) != string::npos) ||
           (code.length() == 2 &&
            string("1234567890").find(code[1]) != string::npos)))
        ;
      else if (boost::regex_match(dict_->name(), boost::regex("^sbjm$")) &&
               code.length() == 3) {
        if (ctx->get_option("third_pop"))
          user_dict_->LookupWords(&uter, code, false);
        else if (!ctx->get_option("slow_adjust") &&
                 string("aeuio").find(code[2]) != string::npos)
          user_dict_->LookupWords(&uter, code, false);
        else
          ;
      } else
      ...
      if (encoder_ && encoder_->loaded()) {
        if (boost::regex_match(user_dict_->name(),
                               boost::regex("^sbjm|sbfx$")) &&
            (code.length() < 3 ||
             (code.length() == 3 && uter.cache_size() == 1)))
          ;  // do nothing
        else if (!ctx->get_option("third_pop") &&
                 boost::regex_match(dict_->name(), boost::regex("^sbjm$")) &&
                 code.length() == 3)
          ;
        else
  ...
  if (translation) {
    bool filter_by_charset =
        enable_charset_filter_ && !ctx->get_option("extended_charset");
    ...
    if (is_constructed(e)) {
      ...
      user_dict_->UpdateEntry(blessed, 1);
               ...
               1 == utf8::unchecked::distance(
                        e->text.c_str(), e->text.c_str() + e->text.length())) {
      ;  // no change for chars
    } else if (boost::regex_match(user_dict_->name(),
      ...
      if (!history.empty()) {
        ...
        for (; it != history.rend(); ++it) {
          if (it->type != "table" && it->type != "user_table" &&
              it->type != "sentence" && it->type != "simplified" &&
              it->type != "uniquified" && it->type != "completion" &&
              it->type != "raw")
            break;
          if (it->type == "raw") {
            if (it->text.empty())
          ...
          if (phrase.empty()) {
            ...
            if (boost::regex_match(
                    user_dict_->name(),
                    boost::regex("^pygd|sbjm|sbzr|sbxh|sbf[mxd]$")) &&
                pos != string::npos) {
  ...
  for (size_t start_pos = 0; start_pos < input.length(); ++start_pos) {
    ...
    if (dict_ && dict_->loaded()) {
      ...
      for (const auto& m : boost::adaptors::reverse(matches)) {
        ...
        size_t end_pos = start_pos + consumed_length;
        auto& homographs = same_start_pos[end_pos];
        if (homographs.size() >= max_entries)
          continue;
        DictEntryIterator iter;
