...
void CommitEntry::AppendPhrase(const an<Phrase>& phrase) {
 ...
 if (phrase->language()
	 && boost::regex_match(phrase->language()->name()
		 , boost::regex("^sss|sb|ss|jm3|sn1|sn2|jmsbb|fmsbb|fmsbbtz|kmsbb|kmsbbtz|shs|szs|sxs|sbs|sbsb|sps|spsb|sys|sysb|sygd|sybb|spgd|spbb$"))) {
...
void Memory::OnCommit(Context* ctx) {
  ...
  for (auto& seg : ctx->composition()) {
	...
	if (phrase && phrase->language() && this && this->language()
		&& boost::regex_match(phrase->language()->name()
			, boost::regex("^sss|sb|ss|jm3|sn1|sn2|jmsbb|fmsbb|fmsbbtz|kmsbb|kmsbbtz|szs|sxs|shs|sbs|sbsb|sps|spsb|sys|sysb|sygd|sybb|spgd|spbb$"))) {
		recognized = true;
	}
    if (recognized) {
