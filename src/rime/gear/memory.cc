...
namespace rime {
	...
	void CommitEntry::AppendPhrase(const an<Phrase>& phrase) {
		if (phrase->language() && boost::regex_match(phrase->language()->name()
				, boost::regex("^sss|sb|ss|jm3|jmn|fmn|fxn|spn|jmsbb|fmsbb|spszb|fmzdy|fxzdy|jmzdy|spzdy|pygd|sxs|sbs|sbsb|sps|spsb|sps_b$"))) {
	...
	void Memory::OnCommit(Context* ctx) {
		...
		for (auto& seg : ctx->composition()) {
			...
			if (boost::regex_match(name
					, boost::regex("^sss|sb|ss|jm3|jmn|fmn|fxn|spn|jmsbb|fmsbb|spszb|fmzdy|fxzdy|jmzdy|spzdy|pygd|sxs|sbs|sbsb|sps|sps_b|spsb$"))) {
				recognized = true;
			}
			if (recognized) {
				if (name == "sbpy") {
