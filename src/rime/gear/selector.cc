...
namespace rime {
    ...
    ProcessResult Selector::ProcessKeyEvent(const KeyEvent& key_event) {
		...
		bool is_sbxlm = boost::regex_match(schema, boost::regex("^sb[fk][mxd]|sb[fkhz]j|sbjm|sbzr|sbsp|sbjp|sbxh|sbpy|sb[fkhzjd]z$"));
			...
			if (is_sbxlm && len > 0 && islower(c1)) {
				if (key_event.ctrl() && (ch >= XK_0 && ch <= XK_9 || ch >= XK_KP_0 && ch <= XK_KP_9))
					...
					if (current_segment.HasTag("paging"))
						;
					else if (boost::regex_match(schema, boost::regex("^sbjm|sbdp|sbjp$"))) {
						return kNoop;
					}
