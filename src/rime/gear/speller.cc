...
ProcessResult Speller::ProcessKeyEvent(const KeyEvent& key_event) {
  ...
  bool pro_char =
      ctx->get_option("pro_char") &&
      boost::regex_match(schema, boost::regex("^sbf[mxd]|sbzr|sbxh$"));
  bool is_buffered =
      ctx->get_option("is_buffered") &&
      boost::regex_match(schema, boost::regex("^sbf[mxd]|sbjm|sbzr|sbxh$"));
  bool is_enhanced =
      ctx->get_option("is_enhanced") &&
      boost::regex_match(schema, boost::regex("^sbf[mxd]|sbjm|sbzr|sbxh$"));
  bool no_num_pop =
      ctx->get_option("no_num_pop") &&
      boost::regex_match(schema, boost::regex("^sbf[md]|sbjm|sbzr|sbxh$"));
  ...
  if (is_initial && pro_char && !ctx->get_option("is_delayed") && 2 == len &&
      is_sbxlm && belongs_to(c1, initials_)) {
    ctx->ConfirmCurrentSelection();
    if (!is_buffered) {
      ctx->Commit();
    }
  ...
  if (isupper(ch) && pro_char && !ctx->get_option("is_delayed") && 2 == len &&
      is_sbxlm && belongs_to(c1, initials_)) {
  ...
  if (3 == len && belongs_to(c1, initials_) &&
    ...
    //	return kAccepted;
    //}
    else if (string("23789").find(ch) != string::npos &&
             ctx->get_option("fast_pop")) {
      ctx->PushInput(ch);
      return kAccepted;
    }
  ...
  if (3 == len && belongs_to(c1, initials_) && ctx->get_option("pro_char") &&
      ctx->get_option("is_delayed") &&
      string("qwrtsdfgzxcvbyphjklnm").find(ctx->input()[comfirmed_pos + 2]) !=
          string::npos &&
  ...
  if (4 == len && belongs_to(c1, initials_) && islower(ch) &&
      string("1234567890").find(ctx->input()[comfirmed_pos + 2]) !=
          string::npos &&
      boost::regex_match(schema, boost::regex("^sbfx$"))) {
    if (is_buffered) {
      ctx->ConfirmCurrentSelection();
  ...
  if (isdigit(ch) && !ctx->get_option("is_enhanced") && 1 <= len &&
      belongs_to(c1, initials_) &&
      boost::regex_match(
          schema,
          boost::regex(
              "^sbf[mxd]|sbjm|sbzr|sbxh|zrzdy|xhzdy|fmzdy|fxzdy|jmzdy$"))) {
    return kNoop;
  }
  ...
  if (isdigit(ch) && is_enhanced && 3 == len && belongs_to(c1, initials_) &&
    ...
    //2); 		ctx->ConfirmCurrentSelection(); 		ctx->set_caret_pos(ctx->caret_pos() +
    //2);
    //	}
    //	else {
  ...
  if (isdigit(ch) && is_enhanced && 4 == len && belongs_to(c1, initials_) &&
      !no_num_pop &&
  ...
  if (is_popped &&
      (ctx->OkSsss() || ctx->OkSssy() || ctx->OkSsy() || ctx->OkSy()) &&
      string("qwrtsdfgzxcvbyphjklnm").find(ch) != string::npos &&
  ...
  if (is_editable) {
    ...
    if (string("aeuio").find(input[comfirmed_pos + len - 1]) != string::npos &&
      ...
      if (i == 5 &&
        ...
        else if (string("aeuio").find(input[comfirmed_pos + i + 1]) ==
                 string::npos)
          ctx->set_caret_pos(comfirmed_pos + i + 1);
        else {
          for (int j = 2; j < 6; j++) {
            if (string("aeuio").find(input[comfirmed_pos + i + j]) ==
                string::npos) {
  ...
  if (4 == len && (isupper(ch) || !ctx->HasMenu()) &&
      belongs_to(c1, initials_) &&
      string("aeuio").find(ctx->input()[comfirmed_pos + 2]) == string::npos &&
      boost::regex_match(schema, boost::regex("^sbf[mxd]|sbzr|sbxh|sbjm$"))) {
    if (isupper(ch) && ctx->get_option("pro_char") &&
        ctx->get_option("is_delayed") &&
        boost::regex_match(schema, boost::regex("^sbfm$"))) {
      if (string("AEUIO").find(ch) == string::npos) {
        ctx->ConfirmCurrentSelection();
...
bool Speller::AutoSelectAtMaxCodeLength(Context* ctx) {
  ...
  if (cand && reached_max_code_length(cand, mcl) &&
...
bool Speller::AutoSelectPreviousMatch(Context* ctx, Segment* previous_segment) {
  ...
  if (5 == len && is_table_entry(cand) &&
      string("QWRTSDFGZXCVBYPHJKLNM").find(input[comfirmed_pos + 3]) ==
          string::npos &&
    ...
    return FindEarlierMatch(ctx, start, end - 1);
             ...
             string("aeuio").find(input[comfirmed_pos + 4]) != string::npos &&
             !(string("aeuio").find(input[comfirmed_pos + 1]) != string::npos &&
               string("qwrtsdfgzxcvbyphjklnm").find(input[comfirmed_pos + 2]) ==
                   string::npos) &&
             !(string("aeuio").find(input[comfirmed_pos + 1]) == string::npos &&
               string("qwrtsdfgzxcvbyphjklnm").find(input[comfirmed_pos + 2]) ==
                   string::npos) &&
             boost::regex_match(schema,
                                boost::regex("^sbf[md]|sbzr|sbxh|sbjm$"))) {
    return FindEarlierMatch(ctx, start, end - 1);
