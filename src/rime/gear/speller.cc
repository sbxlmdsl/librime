...
namespace rime {
    ...
    ProcessResult Speller::ProcessKeyEvent(const KeyEvent &key_event) {
		...
		bool is_enhanced = ctx->get_option("is_enhanced") && boost::regex_match(schema, boost::regex("^sbf[mj]|sbjm|sbsp$"));
		bool no_num_pop = ctx->get_option("no_num_pop") && boost::regex_match(schema, boost::regex("^sbf[mj]|sbjm|sbsp$"));
		...
		if (isdigit(ch) && is_enhanced && 3 == len && belongs_to(c1, initials_) && !no_num_pop && schema == "sbjm" 
