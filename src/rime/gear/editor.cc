...
void Editor::DeleteCandidate(Context* ctx) {
	...
	if (boost::regex_match(schema, boost::regex("^sbjm|sbdp|sbjk|sbjp|sb[hz][js]|sbxh|sbzr|sbsp|sb[fk][jsmx]$"))) {
		...
		if (len >= 3 && string("aeuio").find(ctx->input()[comfirmed_pos + 2]) != string::npos
			&& boost::regex_match(schema, boost::regex("^sb[hz][js]|sbxh|sbzr|sbsp|sbjp|sb[fk][jsmx]$")))
			return;
	}
	ctx->DeleteCurrentSelection();
}
