  ...
  if (ctx->IsComposing()) {
    ...
    if (RIME_STRUCT_HAS_MEMBER(*context, context->commit_text_preview)) {
      ...
      if (!commit_text.empty()) {
        ...
        if (boost::regex_match(session->schema()->schema_id(), boost::regex("^sbjm|sbxh|sbzr|sbjk|sb[fk]m|sbdp|sb[fk]m[ks]|sb[fk][jsx]|sb[hz]s$")) 
  ...
  if (ctx->HasMenu()) {
    ...
    if (page) {
      ...
      for (const an<Candidate> &cand : page->candidates) {
        ...
        if (boost::regex_match(schema->schema_id(), boost::regex("^sbjm|sbxh|sbzr|sbjk|sb[fk]m|sbdp|sb[fk]m[ks]|sb[fk][jsx]|sb[hz]s$"))) {
